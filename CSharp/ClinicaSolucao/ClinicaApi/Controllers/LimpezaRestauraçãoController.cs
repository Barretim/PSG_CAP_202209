...
namespace ClinicaApi.Controllers
{
    ...
    [ApiController]
    public class LimpezaRestauraçãoController : ControllerBase
    {
        ...
        public LimpezaRestauraçãoController(ClinicaContext contexto) : base()
        {
        ...
        public ActionResult<ServicoPoco> Obter(int id)
        {
            try
            {
                List<ServicoPoco> listaPoco = this.servico.Consultar(s => (s.TipoServico == "LR") && (s.CodigoServico == id));
                return Ok(listaPoco);
            }
            catch (Exception ex)
            { 
